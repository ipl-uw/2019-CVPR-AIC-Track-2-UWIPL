...
class BaseDataset(object):
    ...
    def get_imagedata_info_ori(self, data):
        pids, cams = [], []
        for _, pid, camid,orientation in data:
            pids += [pid]
            cams += [camid]
        pids = set(pids)
        cams = set(cams)
    ...
    def get_videodata_info_ori(self, data, return_tracklet_stats=False):
        pids, cams, tracklet_stats = [], [], []
        for img_paths, pid, camid,orientation in data:
            pids += [pid]
            cams += [camid]
            tracklet_stats += [len(img_paths)]
        pids = set(pids)
    ...
    def get_imagedata_info_ori_iou(self, data):
        pids, cams = [], []
        for _, pid, camid,orientation,iou in data:
            pids += [pid]
            cams += [camid]
        pids = set(pids)
        cams = set(cams)
    ...
    def get_videodata_info_ori_iou(self, data, return_tracklet_stats=False):
        pids, cams, tracklet_stats = [], [], []
        for img_paths, pid, camid,orientation,iou in data:
            pids += [pid]
            cams += [camid]
            tracklet_stats += [len(img_paths)]
        pids = set(pids)
...
class BaseVideoDataset(BaseDataset):
    ...
    def print_dataset_statistics_ori(self, train, query, gallery):
        num_train_pids, num_train_tracklets, num_train_cams, train_tracklet_stats = \
            self.get_videodata_info_ori(train, return_tracklet_stats=True)
        
        num_query_pids, num_query_tracklets, num_query_cams, query_tracklet_stats = \
            self.get_videodata_info_ori(query, return_tracklet_stats=True)
        
        num_gallery_pids, num_gallery_tracklets, num_gallery_cams, gallery_tracklet_stats = \
            self.get_videodata_info_ori(gallery, return_tracklet_stats=True)
    ...
    def print_dataset_statistics_ori_iou(self, train, query, gallery):
        num_train_pids, num_train_tracklets, num_train_cams, train_tracklet_stats = \
            self.get_videodata_info_ori_iou(train, return_tracklet_stats=True)
        
        num_query_pids, num_query_tracklets, num_query_cams, query_tracklet_stats = \
            self.get_videodata_info_ori_iou(query, return_tracklet_stats=True)
        
        num_gallery_pids, num_gallery_tracklets, num_gallery_cams, gallery_tracklet_stats = \
            self.get_videodata_info_ori_iou(gallery, return_tracklet_stats=True)
