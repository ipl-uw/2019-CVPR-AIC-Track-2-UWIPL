...
def forward_batch(model, criterion, inputs, targets, opt, phase):
    ...
    #    index = 1
        ...
        # if using compcar dataset, we only train attr 1 (brand)
        # set w=1 for index==1 only
        # otherwise set w=1 for index==0/1/2 to train the three attributes 
        w = 1
        if index == 0:
            w = 0
        elif index == 1:
...
def forward_dataset(model, criterion, data_loader, opt):
    ...
    for i, data in enumerate(data_loader):
        ...
        #output, loss, loss_list = forward_batch(model, criterion, inputs, targets, opt, "Test-train")
        batch_accuracy = calc_accuracy(output, targets, opt.score_thres, opt,opt.top_k)
        #if opt.mode == 'Test':
        #    with torch.no_grad():
        #        for i in range(len(output)):
        #            logging.info(((F.softmax(output[i], dim=1)).cpu()).data.numpy())
        # accumulate accuracy
        if len(accuracy) == 0:
...
def calc_accuracy(outputs, targets, score_thres, opt, top_k=(1,)):
    typelist = ['Sedan','SUV','Truck','Minivan','PickupTruck','Hatchback','Bus']
    brandlist = ['Dodge','Ford','Chevrolet','GMC','Honda','Chrysler','Jeep','Hyundai','Subaru','Toyota','Buick',\
            'KIA','Nissan','Volkswagen','Oldsmobile','BMW','Cadillac','Volvo','Pontiac','Mercury','Lexus',\
            'Saturn','Benz','Mazda','Scion','Mini','Lincoln','Audi','Mitsubishi','others']
    colorlist = ['Black','White','Gray','Blue','Red','Gold','Silver','Green','Yellow']
    ...
    for i in range(len(targets)):        
        ...
        index = index.t()
        p_o.append(((index.numpy())[0])[0])

    p_t = [list(i.cpu().data.numpy())[0] for i in targets]
    #target_label = [typelist[p_t[0]],brandlist[p_t[1]],colorlist[p_t[2]]]
    #pred_label = [typelist[p_o[0]],brandlist[p_o[1]],colorlist[p_o[2]]]
    #logging.info(target_label)
